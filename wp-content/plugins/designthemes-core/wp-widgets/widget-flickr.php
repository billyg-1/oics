<?php 
class Pallikoodam_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		...
		parent::__construct( false, PALLIKOODAM_THEME_NAME.esc_html__(' Flickr Widget','dt-elementor'), $widget_options );
	}
         ...
         <p>
         	<label for="<?php echo esc_attr($this->get_field_id('size'));?>">
         		...
         		<select class="widefat" id="<?php echo esc_attr($this->get_field_id('size')); ?>" name="<?php echo esc_attr($this->get_field_name('size')); ?>"><?php
         			$a = array(
						"s" => esc_html__("square",'dt-elementor'),
						"t" => esc_html__("thumbnail",'dt-elementor'),
						"m" => esc_html__("medium",'dt-elementor')
         			);
