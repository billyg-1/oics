		...
		if($is_shortcode) {
			...
			if( is_shop() ) {
				...
				if($woocommerce_loop['name'] == 'up-sells') {
					...
					$product_style_template = pallikoodam_get_option( 'dt-single-product-related-style-template' );
				} else {
					$product_style_template = pallikoodam_get_option( 'shop-page-product-style-template' );
				}
			}
...
if ( ! function_exists( 'dt_sc_woo_default_product_settings' ) ) {
	...
	function dt_sc_woo_default_product_settings() {
		...
		$settings["template-title"]                                 = esc_html__("Default Template", 'pallikoodam');
		$settings["product-style"]                                  = "product-style-default";
		$settings["product-hover-styles"]                           = "product-hover-thumb-fade-border";
		$settings["product-overlay-bgcolor"]                        = "rgba(0,0,0,0.07)";
		$settings["product-overlay-effects"]                        = "product-overlay-lefttoright";
		$settings["product-hover-image-effects"]                    = "product-hover-image-fadeinleft";
		$settings["product-hover-secondary-image-effects"]          = "product-hover-secimage-pushleft";
		$settings["product-content-hover-effects"]                  = "product-content-hover-slideleft";
		$settings["product-icongroup-hover-effects"]                = "";
		$settings["product-borderorshadow"]                         = "product-borderorshadow-shadow";
		$settings["product-border-type"]                            = "product-border-type-default";
		$settings["product-border-position"]                        = "product-border-position-default";
		$settings["product-shadow-type"]                            = "product-shadow-type-thumb";
		...
		$settings["product-padding"]                                = "product-padding-default";
		...
		$settings["product-thumb-iconsgroup-icons"]                 = array ( 'compare', 'wishlist', 'quickview' );
...
if ( ! function_exists( 'pallikoodam_product_style_setup_template_prop' ) ) {

	function pallikoodam_product_style_setup_template_prop($woo_product_style_template) {
		...
		if(!wc_get_loop_prop('is_shortcode')) {
			...
			if( is_shop() ) {
				$columns = pallikoodam_get_option( 'shop-page-product-layout' );
			} elseif( is_product_category() ) {
				$columns = pallikoodam_get_option( 'dt-woo-category-archive-product-layout' );
			} elseif( is_product_tag() ) {
				$columns = pallikoodam_get_option( 'dt-woo-tag-archive-product-layout' );
			} else {
				$columns = pallikoodam_get_option( 'shop-page-product-layout' );
			}
...
if ( ! function_exists( 'pallikoodam_product_style_reset_template_prop' ) ) {

	function pallikoodam_product_style_reset_template_prop() {
...
if ( ! function_exists( 'pallikoodam_woo_post_display_mode_from_location' ) ) {

	function pallikoodam_woo_post_display_mode_from_location($woo_product_style_template) {
...
if ( ! function_exists( 'pallikoodam_woo_post_class' ) ) {

	function pallikoodam_woo_post_class($classes, $class = '', $post_id = '') {
	 ...
	 add_filter('post_class', 'pallikoodam_woo_post_class', 21, 3 );

}

/** Updating Product Category Loop Class **/

if ( ! function_exists( 'pallikoodam_woo_product_cat_class' ) ) {

	function pallikoodam_woo_product_cat_class($classes = '', $class = '', $category = '') {
	...
	add_filter('product_cat_class', 'pallikoodam_woo_product_cat_class', 21, 3 );
...
if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_cart' ) ) {

	function pallikoodam_woo_loop_product_button_elements_cart() {
		...
		if( !empty($add_to_cart) ) {
			...
			echo '<div class="wcct_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Add To Cart', 'pallikoodam' ).'">'.pallikoodam_html_output($add_to_cart).'</div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_cart', 'pallikoodam_woo_loop_product_button_elements_cart' );

}

// Wishlist

if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_wishlist' ) ) {

	function pallikoodam_woo_loop_product_button_elements_wishlist() {
		...
		if ( shortcode_exists( 'yith_wcwl_add_to_wishlist' ) ) {
			...
			echo '<div class="wcwl_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Wishlist', 'pallikoodam' ).'">'.do_shortcode('[yith_wcwl_add_to_wishlist product_id="' . $product_id . '"]').'</div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_wishlist', 'pallikoodam_woo_loop_product_button_elements_wishlist' );

}

// Quick View

if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_quickview' ) ) {

	function pallikoodam_woo_loop_product_button_elements_quickview() {
		...
		if ( shortcode_exists( 'yith_quick_view' ) ) {
			...
			echo '<div class="wcqv_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Quick View', 'pallikoodam' ).'">'.do_shortcode('[yith_quick_view product_id="' . $product_id . '"]').'</div>';
		
		}

	}

	add_action( 'dt_woo_loop_product_button_elements_quickview', 'pallikoodam_woo_loop_product_button_elements_quickview' );

}

// Compare

if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_compare' ) ) {

	function pallikoodam_woo_loop_product_button_elements_compare() {
		...
		if( class_exists( 'YITH_Woocompare' ) ) {
			...
			$button_text = get_option( 'yith_woocompare_button_text', esc_html__( 'Compare', 'pallikoodam' ) );
			...
			echo '<div class="wccm_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Compare', 'pallikoodam' ).'"><a href="'.esc_url_raw( add_query_arg( $url ) ).'" class="'.esc_attr($class).'" data-product_id="'.esc_attr($product->get_id()).'" rel="nofollow">'.pallikoodam_html_output($button_text).'</a></div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_compare', 'pallikoodam_woo_loop_product_button_elements_compare' );

}

// Swatches

if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_swatches' ) ) {

	function pallikoodam_woo_loop_product_button_elements_swatches() {

		// Swatches
		pallikoodam_woo_loop_product_content_swatches();

	}

	add_action( 'dt_woo_loop_product_button_elements_swatches', 'pallikoodam_woo_loop_product_button_elements_swatches' );

}

// Size Guide

if ( ! function_exists( 'pallikoodam_woo_loop_product_button_elements_sizeguide' ) ) {

	function pallikoodam_woo_loop_product_button_elements_sizeguide() {
		...
		if($single_product_size_guides) {
			...
			echo '<div class="wcsg_btn_wrapper wc_btn_inline" data-tooltip="'.esc_attr__('Size Guide', 'pallikoodam' ).'"><a href="#" class="button dt-wcsg-button" data-product_id="'.esc_attr($product->get_id()).'">'.esc_html__('Size Guide', 'pallikoodam' ).'</a></div>';

		}

	}

	add_action( 'dt_woo_loop_product_button_elements_sizeguide', 'pallikoodam_woo_loop_product_button_elements_sizeguide' );
