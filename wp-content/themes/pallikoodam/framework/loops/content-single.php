<?php
	$global_post_styles = array( 'breadcrumb-fixed', 'breadcrumb-parallax', 'overlay', 'overlap', 'custom' );

	$post_meta = get_post_meta($post->ID,'_dt_post_settings',TRUE);
	$post_meta = is_array($post_meta) ? $post_meta  : array();

	$enable_lightbox = pallikoodam_get_option( 'enable-image-lightbox' );
	...
	if( !empty( $post_style ) && $post_style != 'custom' ): ?>
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
            $template = apply_filters( 'pallikoodam_blog_single_template', 'framework/templates/single/post-'.$post_style.'.php' );
        ...
        <article id="post-<?php the_ID();?>" <?php post_class($post_classes);?>><?php
			...
			edit_post_link( esc_html__( ' Edit ','pallikoodam' ) ); ?>
        </article><?php
	endif;