    <div class="dt-sc-posts-list-wrapper"><?php
        ...
        $post_layout = pallikoodam_get_option( 'blog-post-layout' );
		$combine_class[] = $post_layout.'-layout';

        $post_gl_style = pallikoodam_get_option( 'blog-post-grid-list-style' );
		$post_cover_style = pallikoodam_get_option( 'blog-post-cover-style' );
		$combine_class[] = $post_style = ( $post_layout == 'entry-grid' || $post_layout == 'entry-list' ) ? $post_gl_style.'-style' : $post_cover_style.'-style';

		$post_list_type = pallikoodam_get_option( 'blog-list-thumb' );
		$combine_class[] = ( $post_layout == 'entry-list' ) ? $post_list_type : '';

		$post_equal_height = pallikoodam_get_option( 'enable-equal-height' );
		...
		$post_no_space = pallikoodam_get_option( 'enable-no-space' );
        ...
        if( have_posts() ):
            ...
            $meta = $obj->pallikoodam_post_meta_fields(false);
        ...
        <!-- **Pagination** --><?php
        $pagination = pallikoodam_get_option( 'blog-pagination' );
        if( $pagination == 'older_newer' ): ?>
            ...
            <div class="pagination blog-pagination">
                ...
                if(get_previous_posts_link()):
                    ?>
                    <div class="newer-posts"><?php echo get_previous_posts_link( '<i class="fas fa-angle-left"></i>'.esc_html__(' Newer Posts', 'pallikoodam') ); ?></div>
                    <?php
                endif;
                if(get_next_posts_link()):
                    ?>
                    <div class="older-posts"><?php echo get_next_posts_link( esc_html__('Older Posts ', 'pallikoodam').'<i class="fas fa-angle-right"></i>' ); ?></div>
                    <?php
                endif;
                ?>
            </div><?php
        ...
        elseif( $pagination == 'numbered' ): ?>
            ...
            <div class="pagination blog-pagination"><?php echo pallikoodam_pagination(); ?></div><?php
        ...
        elseif( $pagination == 'infinite_scroll' ):
            ...
            <div class="pagination blog-pagination"><div class="infinite-btn more-items" data-maxpage="<?php echo esc_attr($wp_query->max_num_pages); ?>" data-pos="<?php echo esc_attr($pos); ?>" data-eheight="<?php echo esc_attr($post_equal_height); ?>" data-style="<?php echo esc_attr($post_style); ?>" data-layout="<?php echo esc_attr($post_layout); ?>" data-column="<?php echo esc_attr($post_columns); ?>" data-listtype="<?php echo esc_attr($post_list_type); ?>" data-hover="<?php echo esc_attr($post_img_hover_style); ?>" data-overlay="<?php echo esc_attr($post_img_overlay_style); ?>" data-align="<?php echo esc_attr($post_alignment); ?>" data-meta=""></div></div><?php
