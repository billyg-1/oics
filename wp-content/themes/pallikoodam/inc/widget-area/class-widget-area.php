<?php
/**
 * PALLIKOODAM Theme Widget Areas
 */

if ( ! class_exists( 'PALLIKOODAM_Widget_Areas' ) ) {

	class PALLIKOODAM_Widget_Areas {
				...
				<div id="dttheme-add-widget" class="widgets-holder-wrap">
					<div>
						<div class="sidebar-name">
							<h3><?php esc_html_e( 'Create Widget Area', 'pallikoodam' ); ?>
			  			...
			  			<div class="sidebar-description">
			  				<form method="post">
			  					<div class="widget-content">
			  						<input id="dttheme-add-widget-input" name="dttheme-add-widget-input" type="text" class="regular-text" title="<?php esc_attr_e( 'Name', 'pallikoodam' ); ?>" placeholder="<?php esc_attr_e( 'Name', 'pallikoodam' ); ?>" />
			  					</div>
			  					<div class="widget-control-actions">
			  						<div class="aligncenter">
			  							<input class="button-primary" type="submit" value="<?php esc_attr_e( 'Create Widget Area', 'pallikoodam' ); ?>" />
			  						</div>
									<br class="clear">
								</div>
							</form>
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'pallikoodam-widget-areas-js', PALLIKOODAM_THEME_URI . '/inc/widget-area/assets/js/widget-areas.js', array('jquery'), PALLIKOODAM_THEME_VERSION, true );
			wp_enqueue_style( 'pallikoodam-widget-areasr-css', PALLIKOODAM_THEME_URI . '/inc/widget-area/assets/css/widget-areas.css', null, PALLIKOODAM_THEME_VERSION );
			wp_localize_script( 'pallikoodam-widget-areas-js', 'dtObject', array(
				'delete'  => esc_html__( 'Delete', 'pallikoodam' ),
				'confirm' => esc_html__( 'Confirm', 'pallikoodam' ),
				'cancel'  => esc_html__( 'Cancel', 'pallikoodam' ),
			) );
		}
		...
		public function save_widget_areas() {
			$options = get_option( PALLIKOODAM_THEME_SETTINGS, array() );
			...
			update_option( PALLIKOODAM_THEME_SETTINGS, $options );
		}

		public function register_main_widget_areas() {
			// Define sidebars array
			$sidebars = array(
				'standard-sidebar-left'  => esc_html__( 'Standard | Left Sidebar', 'pallikoodam' ),
				'standard-sidebar-right' => esc_html__( 'Standard | Right Sidebar', 'pallikoodam' ),
			);

			if( class_exists('Tribe__Events__Main')	):
				$sidebars['events-everywhere-sidebar-left'] = esc_html__('Events | Left Sidebar', 'pallikoodam');
				$sidebars['events-everywhere-sidebar-right'] = esc_html__('Events | Right Sidebar', 'pallikoodam');
			endif;
			...
			switch($page_layout) :
				case 'with-left-sidebar':
					$sidebars['post-archives-sidebar-left'] = esc_html__("Post Archives | Left Sidebar",'pallikoodam');
				break;
		
				case 'with-right-sidebar':
					$sidebars['post-archives-sidebar-right'] = esc_html__("Post Archives | Right Sidebar",'pallikoodam');
				break;
			...
			if ( $sidebars ) {
				...
				$tag   = pallikoodam_get_option( 'widget-title-tag', 'h3' );
				$title = pallikoodam_get_option( 'widget-title-style' );
...
PALLIKOODAM_Widget_Areas::get_instance();