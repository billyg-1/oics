    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. esc_html__( 'Error! Can not load json file.', 'pallikoodam' ) .'</h4>';
        }
...
if( ! function_exists( 'dtm_set_icons' ) ) {
  function dtm_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. esc_attr__( 'Add Icon', 'pallikoodam' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. esc_attr__( 'Search a Icon...', 'pallikoodam' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. esc_html__( 'Loading...', 'pallikoodam' ) .'</div></div>';
    echo '</div>';
