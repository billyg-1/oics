<?php
if ( ! class_exists( 'Pallikoodam_Dynamic_CSS' ) ) {

	class Pallikoodam_Dynamic_CSS {

		private static $instance;

		public $tablet_portrait_breakpoint = "@media only screen and (min-width:992px) and (max-width:1199px)";
		public $tablet_ls_breakpoint = "@media only screen and (min-width:768px) and (max-width:991px)";
		...
		public function enqueue_styles() {
            ...
            if( !empty( $css ) ) {
				...
				wp_register_style( 'pallikoodam-customiser-inline', '', array(), PALLIKOODAM_THEME_VERSION, 'all' );
				wp_enqueue_style( 'pallikoodam-customiser-inline' );
                wp_add_inline_style( 'pallikoodam-customiser-inline', $css );
            }
		}
		...
		public function font_family_responsive_callback( $option, $selector ) {
			...
			# Line Height
				...
				if( isset( $option['lh-tablet'] ) && !empty( $option['lh-tablet'] ) ) {
					$tablet_portrait_css .= 'line-height:'.$option['lh-tablet'].'px;';
				}

				# Tablet landscape  
				if( isset( $option['lh-tablet-landscape'] ) && !empty( $option['lh-tablet-landscape'] ) ) {
					$tablet_ls_css .= 'line-height:'.$option['lh-tablet-landscape'].'px;';
				}

				# Mobile  
				if( isset( $option['lh-mobile'] ) && !empty( $option['lh-mobile'] ) ) {
					$mobile_css .= 'line-height:'.$option['lh-mobile'].'px;';
				}
		...
		public function body_style() {
			...
			$layout = pallikoodam_get_option( 'site-layout' );
			$customize = pallikoodam_get_option( 'site-boxed-layout', true );

			if( $layout && $customize ) {

				$css .= $this->background_callback( pallikoodam_get_option('site-bg') );
				if( !empty( $css ) ) {
			...
			if( !empty( $body_bg_color ) ) {
				...
				$layout = pallikoodam_get_option( 'site-layout', 'wide' );
		...
		public function menu_style() {
			...
			# Menu Color
			$menu_color = pallikoodam_get_option( 'menu-color' );
			if( !empty( $menu_color ) ) {
				$css .= '.no-header-menu ul li a, .dt-header-menu .dt-primary-nav li a, .elementor-widget-wp-widget-nav_menu > .elementor-widget-container .dt-primary-nav li a { color:'.$menu_color.';}'. "\n";
			}
		...
		public function heading_style() {
			...
			# H1 Color
			$h1_color = pallikoodam_get_option( 'h1-color' );
			if( !empty( $h1_color ) ) {
				$css .= 'h1 { color:'.$h1_color.';}'. "\n";
			}

			# H2 Color
			$h2_color = pallikoodam_get_option( 'h2-color' );
			if( !empty( $h2_color ) ) {
				$css .= 'h2 { color:'.$h2_color.';}'. "\n";
			}

			# H3 Color
			$h3_color = pallikoodam_get_option( 'h3-color' );
			if( !empty( $h3_color ) ) {
				$css .= 'h3 { color:'.$h3_color.';}'. "\n";
			}

			# H4 Color
			$h4_color = pallikoodam_get_option( 'h4-color' );
			if( !empty( $h4_color ) ) {
				$css .= 'h4 { color:'.$h4_color.';}'. "\n";
			}

			# H5 Color
			$h5_color = pallikoodam_get_option( 'h5-color' );
			if( !empty( $h5_color ) ) {
				$css .= 'h5 { color:'.$h5_color.';}'. "\n";
			}

			# H6 Color
			$h6_color = pallikoodam_get_option( 'h6-color' );
			if( !empty( $h6_color ) ) {
		...
		public function menu_typo() {
			...
			# Menu
				$menu_typo =  pallikoodam_get_option( 'menu-typo' );
				...
				# Generating CSS
					...
					if( !empty( $menu_css ) ) {
						$css .= '.no-header-menu ul li a, .dt-header-menu .dt-primary-nav li a, .elementor-widget-wp-widget-nav_menu > .elementor-widget-container .dt-primary-nav li a {'.$menu_css.'}'."\n";
					}
		...
		public function breadcrumb_style() {
			...
			$global_style = pallikoodam_get_option( 'breadcrumb-bg' );
			$overlay = pallikoodam_get_option( 'breadcrumb-overlay-bg-color' );
			...
			# Title Color
				$title_color = pallikoodam_get_option( 'breadcrumb-title-color' );
				if( !empty( $title_color ) ) {
					$css .= '.main-title-section h1 { color:'.$title_color.';}';
				}

			# Text Color
				$text_color = pallikoodam_get_option( 'breadcrumb-text-color' );
				if( !empty( $text_color ) ) {
					$css .= '.breadcrumb span.current { color:'.$text_color.';}';
				}

			# Link Color
				$link_color = pallikoodam_get_option( 'breadcrumb-link-color' );
				if( !empty( $link_color ) ) {
					$css .= '.breadcrumb a { color:'.$link_color.';}';
				}

			# Link Hover Color
				$link_h_color = pallikoodam_get_option( 'breadcrumb-link-h-color' );
				if( !empty( $link_h_color ) ) {
					$css .= '.breadcrumb a:hover { color:'.$link_h_color.';}';
				}
		...
		public function breadcrumb_typo() {
			...
			# Title
				$title_typo =  pallikoodam_get_option( 'breadcrumb-title-typo' );
				...
				# Generating Responsive CSS
					$title_responsive_css = $this->font_family_responsive_callback(  pallikoodam_get_option( 'breadcrumb-title-typo' ), '.main-title-section h1' );
					if( !empty( $title_responsive_css ) ) {
						$css .= $title_responsive_css;
					}

			# Breadcrumb
				$breadcrumb_typo =  pallikoodam_get_option( 'breadcrumb-typo' );
				...
				# Generating Responsive CSS
					$breadcrumb_responsive_css = $this->font_family_responsive_callback(  pallikoodam_get_option( 'breadcrumb-typo' ), '.breadcrumb' );
		...
		public function footer_section_style() {
			...
			# Footer Title
			$footer_title_color = pallikoodam_get_option( 'footer-title-color' );
			if( !empty( $footer_title_color ) ) {
				$css .= 'div.footer-widgets .widgettitle, #footer .widgettitle { color:'.$footer_title_color.';}'. "\n";
			}

			# Footer Content
			$footer_content_color = pallikoodam_get_option( 'footer-content-color' );
			...
			$footer_content_a_color = pallikoodam_get_option( 'footer-content-a-color' );
			if( !empty( $footer_content_a_color ) ) {
				$css .= '.footer-widgets a, #footer a { color:'.$footer_content_a_color.';}'. "\n";				
			}

			# Footer Content Anchor Hover
			$footer_content_a_hover_color = pallikoodam_get_option( 'footer-content-a-hover-color' );
		...
		public function footer_section_typo() {
			...
			# Footer Title
				$footer_title_typo =  pallikoodam_get_option( 'footer-title-typo' );
			...
			# Footer Content 
				$footer_content_typo =  pallikoodam_get_option( 'footer-content-typo' );
		...
		public function widget_style() {
			...
			# Widget Title
				$widget_title_color = pallikoodam_get_option( 'widget-title-color' );

				if( !empty( $widget_title_color ) ) {
					$css .= '.secondary-sidebar .widgettitle { color:'.$widget_title_color.' }'. "\n";
				}

			# Wdiget Title Border and Background Color
				$widget_title_style = pallikoodam_get_option( 'widget-title-style' );
				$widget_title_border_color = pallikoodam_get_option( 'widget-title-border-color' );
				$widget_title_bg_color = pallikoodam_get_option( 'widget-title-bg-color' );
				...
				if( $widget_title_style == 'type1' || $widget_title_style == 'type3' || $widget_title_style == 'type4' || $widget_title_style == 'type5' || $widget_title_style == 'type6' || $widget_title_style == 'type7' || $widget_title_style == 'type8' || $widget_title_style == 'type9' || $widget_title_style == 'type10' || $widget_title_style == 'type13' || $widget_title_style == 'type14' || $widget_title_style == 'type16') {
					...
					if( !empty( $widget_title_border_color ) ) {
						$css .= '.secondary-sidebar .type1 .widgettitle:before, .secondary-sidebar .type1 .widgettitle:after, .secondary-sidebar .type7 .widgettitle:before, .secondary-sidebar .type9 .widgettitle:before, .secondary-sidebar .type10 .widget:before { background-color:'.$widget_title_border_color.' }'. "\n";
			...
			# Widget Background Color
				$widget_bg_color = pallikoodam_get_option( 'widget-bg-color' );
			...
			# Widget Content
				$widget_content_color = pallikoodam_get_option( 'widget-content-color' );

				if( !empty( $widget_content_color ) ) {
						$css .= '.secondary-sidebar .widget, .secondary-sidebar .widget ul li > a, .secondary-sidebar .widget ul li > span > a { color:'.$widget_content_color.';}'. "\n";
				}

			# Widget Content Anchor
				$widget_content_a_color = pallikoodam_get_option( 'widget-content-link-color' );

				if( !empty( $widget_content_a_color ) ) {
					$css .= '.secondary-sidebar .widget .recent-posts-widget li .entry-title h4 a, .secondary-sidebar .widget .recent-posts-widget li .entry-meta a, .secondary-sidebar .widget.widget_rss ul li > a, .secondary-sidebar .widget.widget_recent_comments ul li > a, .secondary-sidebar .widget.widget_recent_comments ul li > span > a { color:'.$widget_content_a_color.';}'. "\n";
				}

			# Widget Content Anchor Hover
				$widget_content_a_h_color = pallikoodam_get_option( 'widget-content-link-h-color' );
		...
		public function widget_typo() {
			...
			# Widget Title
				$widget_title_typo =  pallikoodam_get_option( 'widget-title-typo' );
			...
			# Widget Content
				$widget_content_typo =  pallikoodam_get_option( 'widget-content-typo' );
		...
		public function site_identity_style() {
			...
			# Site Title Color
			$site_title_color = pallikoodam_get_option( 'site-title-color' );
			if( !empty( $site_title_color ) ) {
				$css .= '.site-title a { color:'.$site_title_color.'; }'. "\n";
			}

			# Site Title Color
			$site_title_hcolor = pallikoodam_get_option( 'site-title-h-color' );
			if( !empty( $site_title_hcolor ) ) {
				$css .= '.site-title a:hover { color:'.$site_title_hcolor.'; }'. "\n";
			}

			# Site Tagline Color
			$site_tagline_color = pallikoodam_get_option( 'site-tagline-color' );
		...
		public function site_identity_typo() {
			...
			# Site Title
				$site_title_typo =  pallikoodam_get_option( 'site-title-typo' );
			...
			# Site Tagline
				$site_tagline_typo =  pallikoodam_get_option( 'site-tagline-typo' );
...
Pallikoodam_Dynamic_CSS::get_instance();