			...
			if( $settings['single-post-style'] == 'breadcrumb-fixed' ) {
				...
				echo '<div class="single-post-header-wrapper aligncenter '.$post_cls.'">';
					...
					echo '<div class="container">';
                        ...
                        <div class="post-meta-data">
                            <div class="post-categories"><?php pallikoodam_get_template( $template, $template_args ); ?></div>
                            <div class="date"><?php echo sprintf( esc_html__( '%s ago', 'pallikoodam' ), human_time_diff( get_the_date('U'), current_time('timestamp') ) ); ?></div>
						</div><?php
					echo '</div>';
				...
				echo '<div class="single-post-header-wrapper dt-parallax-bg aligncenter '.$post_cls.'">';
					...
					echo '<div class="container">';
                        ...
                        <div class="post-categories"><?php pallikoodam_get_template( $template, $template_args ); ?></div><?php
                        ...
                        <div class="post-meta">
							<div class="post-author"><?php
								...
								echo '<span>'.esc_html__('By ', 'pallikoodam').'</span>';
                            	echo '<a href="'.get_author_posts_url($authid).'" title="'.esc_attr__('View all posts by ', 'pallikoodam').get_the_author_meta('display_name', $authid).'">'.get_the_author_meta('display_name', $authid).'</a>'; ?>
                            </div>

							<div class="post-date"><?php
								$template = 'framework/templates/single/entry-date.php';
								pallikoodam_get_template( $template, $template_args ); ?></div>

                            <div class="post-comments"><?php
								if( ! post_password_required() ) {
									comments_popup_link( esc_html__('No Comments', 'pallikoodam'), esc_html__('1 Comment', 'pallikoodam'), esc_html__('% Comments', 'pallikoodam'), '', esc_html__('Comments Off', 'pallikoodam'));
								} else {
									echo esc_html__('Enter your password to view comments.', 'pallikoodam');
								} ?>
                            </div>
                        </div><?php
					echo '</div>';
        ...
        if ( $show_sidebar ) {
            if ( $show_left_sidebar ) {
                ...
                <section id="secondary-left" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    pallikoodam_show_sidebar( 'post', $post->ID, 'left' ); ?></section><!-- Secondary Left End --><?php
            }
        }?>
        ...
        if ( $show_sidebar ) {
            if ( $show_right_sidebar ) {
                ...
                <section id="secondary-right" class="secondary-sidebar <?php echo esc_attr( $sidebar_class.$sticky_class );?>"><?php
                    pallikoodam_show_sidebar( 'post', $post->ID, 'right' ); ?></section><!-- Secondary Right End --><?php
            }
        }?>
    </div>
    <!-- ** Container End ** -->
